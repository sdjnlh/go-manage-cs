...
func (mgr *SessionMgr) StartSession(w http.ResponseWriter, r *http.Request) string {
	...
	fmt.Print("start time: ", session.mLastTimeAccessed)
	//让浏览器cookie设置过期时间
	cookie := http.Cookie{Name: mgr.mCookieName, Value: newSessionID, Path: "/", HttpOnly: true, MaxAge: int(1000000)}
...
// 获取用户iD
func (mgr *SessionMgr) GetUserId(c *gin.Context) (id string) {
...
// 获取用户名
func (mgr *SessionMgr) GetUserName(c *gin.Context) string {
	var cookie, err = c.Request.Cookie(mgr.mCookieName)
	if cookie == nil ||
		err != nil {
		c.AbortWithStatus(http.StatusUnauthorized)
...
func (mgr *SessionMgr) CheckCookieValid(c *gin.Context) {
	var cookie, err = c.Request.Cookie(mgr.mCookieName)
	...
	sessionID := cookie.Value
	if session, ok := mgr.mSessions[sessionID]; ok {
		fmt.Print("update up", session.mLastTimeAccessed)
		session.mLastTimeAccessed = time.Now() //判断合法性的同时，更新最后的访问时间
		fmt.Print("update down", mgr.mSessions[sessionID].mLastTimeAccessed)
		c.Next()
...
func (mgr *SessionMgr) GC() {
	...
	for sessionID, session := range mgr.mSessions {
		//删除超过时限的session
		fmt.Print("gc ", session.mLastTimeAccessed)
		fmt.Print(time.Now())
		if session.mLastTimeAccessed.Unix()+mgr.mMaxLifeTime < time.Now().Unix() {
			//val := session.mValues["UserInfo"].(*model.User)
			//val.Status = 2
			//if _, err := app.Service.DB.Table("user").Update(val); err != nil {
			//	delete(mgr.mSessions, sessionID)
			//} else {
			//	continue
			//}
